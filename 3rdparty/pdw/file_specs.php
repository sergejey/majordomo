...
switch($type)
{
	case 'image':
      ...
      $file_size = $file_size < 1024  ? $file_size . ' ' . translate('bytes') : $file_size < 1048576 ? number_format($file_size / 1024, 2, $dec_seperator, $thousands_separator) . ' ' . translate('kB') : number_format($file_size / 1048576, 2, $dec_seperator, $thousands_separator) . ' '.translate('MB');
		$uploadPathArr = explode("/", $uploadpath);
      $filename = array_pop($uploadPathArr);
	...
	case 'file':
      ...
      $uploadFileNameArr = explode("/", $uploadpath);
      $uploadFileExtArr = explode(".", $uploadpath);

      $filename = array_pop($uploadFileNameArr);
		$fileext = array_pop($uploadFileExtArr);
	...
	default: // folder
      ...
      $uploadFolderNameArr = explode("/", trim($uploadpath, "/"));
		$foldername = array_pop($uploadFolderNameArr);
