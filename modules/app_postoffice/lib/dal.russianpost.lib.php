...
namespace DAL
{
   ...
   {
      const FLAG_ACTIVE_TRACK   = "Y";   // ќтслеживать трек-номер
      const FLAG_INACTIVE_TRACK = "N";   // Ќе отслеживать трек-номер
      
      /**
       * ¬озвращает список треков
       * @return array
       */
         ...
         $trackName)
      {
         ...
         if(
            // подготовливаем запрос, там куда будут вствл€тс€ данные отмечаем символом ? (плейсхолдоры)
         ($stmt->prepare("insert into POST_TRACK(TRACK_ID, TRACK_NAME, FLAG_CHECK, TRACK_DATE, LM_DATE)
                          values(?, ?,'Y',NOW(),NOW())") === FALSE)
          // прив€зываем переменные к плейсхолдорам
         or ($stmt->bind_param('ss', $trackID, $trackName) === FALSE)
         
         // отрправл€ем даные, которые на данный момент наход€тс€ в прив€занных переменных
      ...
      /**
       * ¬озращает true/false в зависимости от того используем мы прокси или нет 
       * @return true/false
       */
      public static function isProxy()
      {
         ...
         // отрправл€ем даные, которые на данный момент наход€тс€ в прив€занных переменных
         or ($stmt->execute() === FALSE)
         // прив€зывем переменую дл€ получени€ в нее результата
         or ($stmt->bind_result($isProxy) === FALSE)
         // получение результата в прив€занную переменную
         or ($stmt->fetch() === FALSE)
         // закрываем подготовленный запрос
         or ($stmt->close() === FALSE)
         ) 
