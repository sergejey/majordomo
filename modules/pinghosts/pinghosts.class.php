...
// Create image dimensions
        ...
        if ($is_online) {
            $fill_color = imagecolorallocate($img, 0, 204, 0);
        } else {
            $fill_color = imagecolorallocate($img, 204, 0, 0);
        }
        imagefilledellipse($img, $width / 2, $height / 2, $width - 4, $height - 4, $fill_color);
        imagepng($img);
        imagedestroy($img);
        exit;
