...
$tmp = SQLSelectOne('SELECT COUNT(*) as TOTAL FROM commands');
if ($tmp['TOTAL']) {
    $out['HAVE_MENU'] = 1;
}

$tmp = SQLSelectOne('SELECT COUNT(*) as TOTAL FROM scripts');

if ($tmp['TOTAL'])
{
   $out['HAVE_SCRIPTS'] = 1;
   $res   = SQLSelect('SELECT scripts.*, script_categories.TITLE as CATEGORY
                  FROM scripts
...
$tmp = SQLSelectOne('SELECT COUNT(*) as TOTAL FROM classes');

if ($tmp['TOTAL'])
{
   $out['HAVE_OBJECTS'] = 1;
   $classes = SQLSelect('SELECT * FROM classes ORDER BY PARENT_ID, TITLE');
   $total   = count($classes);
   ...
   for ($i = 0; $i < $total; $i++)
   {
      ...
      {
         $classes[$i]['PARENT_TITLE'] = current(SQLSelectOne('SELECT TITLE FROM classes AND ID = "' . $classes[$i]['PARENT_ID'] . '"'));
      }
      $properties = SQLSelect('SELECT * FROM properties AND CLASS_ID  =  "'. $classes[$i]['ID'] .'" AND OBJECT_ID = 0');

      if ($properties[0]['ID']) {
          $classes[$i]['PROPERTIES'] = $properties;
      }
      $methods = SQLSelect('SELECT * FROM methods AND CLASS_ID  = "' . $classes[$i]['ID'] . '" AND OBJECT_ID = 0');
      ...
      $objects = SQLSelect('SELECT * FROM objects AND CLASS_ID = "' . $classes[$i]['ID'] . '"');
      if ($objects[0]['ID'])
      {
         ...
         {
            $methods = SQLSelect('SELECT * FROM methods WHERE OBJECT_ID = "' . $objects[$io]['ID'] . '"');
            if ($methods[0]['ID'])
            {
               $totalm = count($methods);
...
$tmp = SQLSelectOne('SELECT COUNT(*) as TOTAL FROM patterns');

if ($tmp['TOTAL'])
{
   $out['HAVE_PATTERNS'] = 1;
   $patterns             = SQLSelect('SELECT * FROM patterns ORDER BY PARENT_ID, TITLE');
