...
function processLine($line, $hash = '')
{
   ...
   elseif (preg_match('/^CLEAR (.+?) (\d+) DAYS OLD$/is', $line, $matches)) 
   {
      ...
      if ($days>0) walk_dir($from, "remove_old_files", $days);
   } 
...
function copyTree($source, $destination, $over = 0) 
{
   ...
   if (!is_dir($source)) return 0; // incorrect source path

   if (!is_dir($destination)) 
   {
      if (!mkdir($destination, 0777)) return 0; // cannot create destination path
   }
